...
##Explanation, runs model in eval mode
def gradcam(loader, model, rank, out=False):
    model.eval()
#     for data in loader:
#         data = data.to(rank)
#         with torch.no_grad():
#             output = model(data, gradcam=True)
#             if out == True:
#                 if count == 0:
#                     ids = [item for sublist in data.structure_id for item in sublist]
#                     ids = [item for sublist in ids for item in sublist]
...
#     #with torch.no_grad():
#     print(model)
#     predict = model(data)#, gradcam="True")
...
def explain(dataset, job_parameters=None):
    ...
    ##GradCAM
    time_start = time.time()
    heatmap = gradcam(loader, model, rank, out=True)
    elapsed_time = time.time() - time_start
    print("Explanation time (s): {:.5f}".format(elapsed_time))
    
    ##Write output
    if job_parameters["write_output"] == "True":
        print(heatmap.shape)
        print(type(heatmap))
        np.savetxt(str(job_parameters["job_name"]) + "_explain_outputs.csv", heatmap)
#         write_results(
#             heatmap, str(job_parameters["job_name"]) + "_explain_outputs.csv"
#         )
...
def optuna_setup(
    ...
    val_error = optuna_trainable(config=config, data_path=data_path)
...
def optuna_trainable(config, checkpoint_dir=None, data_path=None):
    print("Hyperparameter trial start")
    hyper_args = config["hyper_args"]
    job_parameters = config["job_parameters"]
    processing_parameters = config["processing_parameters"]
